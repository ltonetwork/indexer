...
export class HashService {
  ...
  async trigger(): Promise<void> {
    ...
    await Promise.all(chunks.map((chunk) => this.node.anchorAll(...chunk).catch(() => undefined)));
  }
  ...
  private processMappedAnchor(
    ...
    const { hash: hashHex, id } = set.sort((a, b) => b.blockHeight - a.blockHeight || b.position - a.position)[0] || {
      hash: null,
      id: null,
    };
